...
class FovConverter:
    ...
    def transform(self, fov_image):
        with torch.no_grad():
            transform_list = [transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
            transform_list = [transforms.Resize(256)] + transform_list
            transform = transforms.Compose(transform_list)

            pointcloud = transform(fov_image).unsqueeze(0).cuda()
        ...
        fov_image_output = outputs.data.detach().cpu().numpy()  # to numpy array

        return fov_image_output[0, 0, :, :]
